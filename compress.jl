...
# ╔═╡ f65b2036-8b1c-4875-bd8d-d9d6a5967eff
tolerance = 1e-5

# ╔═╡ 4b6a17b2-502e-407a-bbab-f22e1c84604f
begin
    # Replace this line with the dataset to be tested for compressibility.
    grid_tt = range(-pi, pi; length = 200)
    dataset_tt = [cos(x) + cos(y) + cos(z) for x in grid_tt, y in grid_tt, z in grid_tt]
    size(dataset_tt)
end

# ╔═╡ c348ba58-58d3-4b58-a512-45cf1c223dd5
md"""
We now construct a TCI."""

# ╔═╡ 8c5ce2f9-0fc1-4bff-b24d-8fcee25733ca
errors_tt = let
    # Construct TCI
    tt, ranks, errors_tt = TCI.crossinterpolate2(
        Float64, i -> dataset_tt[i...], collect(size(dataset_tt)), tolerance = tolerance)

    # Check error
    ttdataset = [tt([i, j, k])
                 for i in axes(grid_tt, 1), j in axes(grid_tt, 1), k in axes(grid_tt, 1)]
    errors_tt = abs.(ttdataset .- dataset_tt)
    println(
        "TCI of the dataset_tt with tolerance $tolerance has link dimensions $(TCI.linkdims(tt)), "
        *
        "for a max error of $(maximum(errors_tt))."
    )
    errors_tt
end;

# ╔═╡ f3c6a898-c440-43c3-8bb4-2be1101d5fd4
md"""
Let us plot the original data and the TCI error on a 2D cut."""

# ╔═╡ f52ef908-eb78-404d-98ab-461447368fce
let
    # Original data
    c1 = heatmap(dataset_tt[:, :, 1], aspect_ratio = 1)
    title!("Original data")

    # TCI error
    c2 = heatmap(log10.(abs.(errors_tt[:, :, 1])), aspect_ratio = 1)
    title!("log10 of abs error of TCI")

    plot(c1, c2, size = (800, 500))
end

# ╔═╡ 2b274d67-bd87-4de9-9565-403ad8c49959
md"""
## QTCI

We now demonstrate how to compress existing data by QTCI."""

# ╔═╡ dc961a12-5d1e-488e-815a-d47e12defd86
begin
    # Number of bits
    R = 8

    # Replace with your dataset
    grid_qtt = range(-pi, pi; length = 2^R + 1)[1:(end - 1)] # exclude the end point
    dataset_qtt = [cos(x) + cos(y) + cos(z) for x in grid_qtt, y in grid_qtt, z in grid_qtt]
    size(dataset_qtt)
end

# ╔═╡ 90ca07b8-cd64-4bd0-99af-aa1389766fbc
md"""
### QuanticsTCI.jl
Let us first use `quanticscrossinterpolate` function in `QuanticsTCI.jl`."""

# ╔═╡ 421840b6-b506-46fd-937f-817af34c68ce
begin
    # Perform QTCI
    qtt, ranks, _ = quanticscrossinterpolate(
        dataset_qtt, tolerance = tolerance, unfoldingscheme = :fused)
end

# ╔═╡ cbd3a596-98dc-4d1a-9029-b7201da835bc
md"""
Below, we compute the error for the whole tensor, which may be too expensive for a large $\mathcal{R}$."""

# ╔═╡ d9a6f697-8fbc-44c2-a2d2-1f343757090c
begin
    # Check error
...
# ╔═╡ db0507c0-cf9c-4f12-bcec-ab703974c522
md"""
Again, let us plot the original data and the TCI error on a 2D cut."""

# ╔═╡ 9218abf3-28c6-4dfd-afdf-413b469abdc8
let
    # Original data
...
# ╔═╡ 62980e80-7283-4f91-a6fc-18cffb930f82
md"""
...
# ╔═╡ 4d02437a-b0f7-4c4e-a7d0-64bc5ee6cbd3
...
# ╔═╡ 8afc5a4d-5b82-44db-8d96-f105e05d36a0
begin
...
# ╔═╡ 083f027f-1b5a-48a0-8993-24f2ff098332
md"""
...
# ╔═╡ cbe34f12-f0a5-4015-ba0e-5c2763685a56
md"""
...
# ╔═╡ fe224fa0-8f49-43f3-923f-d18ef2124a60
initialpivots, localdims = let
    # Local dimensions
    localdims = fill(8, R)
    ...
    initialpivots = [TCI.optfirstpivot(qf, localdims, [rand(1:d) for d in localdims])
                     for _ in 1:ninitialpivots]

    for p in initialpivots
        println("Initial pivot: $p $(qf(p))")
    end
    initialpivots, localdims
end

# ╔═╡ 572d920b-8f5b-4b1e-a13e-3eb308766120
let
    # Perform (Q)TCI
    qtt, _, _ = TCI.crossinterpolate2(
        Float64, qf, localdims, initialpivots; tolerance = tolerance)
    # Test error
...
QuanticsGrids = "~0.3.2"
QuanticsTCI = "~0.7.0"
TensorCrossInterpolation = "~0.9.12"
"""
...
manifest_format = "2.0"
project_hash = "baa0d9b94cf6da178391afa33bdd8dd959f00964"
...
git-tree-sha1 = "699f7b4aeb55ce97f06eb859594a6e6b983486e2"
uuid = "634c7f73-3e90-4749-a1bd-001b8efc642d"
version = "0.3.2"

[[deps.QuanticsTCI]]
deps = ["LinearAlgebra", "QuanticsGrids", "TensorCrossInterpolation"]
git-tree-sha1 = "2724301b6bc6390d8d43d210cecd7dbaabe519d5"
...
deps = ["EllipsisNotation", "LinearAlgebra", "QuadGK"]
git-tree-sha1 = "c08a9ff7144b009c8bb16033a1e5bf88cdb10bf0"
