...
# ╔═╡ d85608d8-93ff-4875-b187-15467bb35765
begin
    using Printf
...
# ╔═╡ 404dabef-0813-4623-84d3-1fe8dfa4b481
md"""
...
# ╔═╡ d3c8602d-9093-4944-98c4-dbd6789d2957
function f(x, y)
    (exp(-0.4 * (x^2 + y^2)) + 1 + sin(x * y) * exp(-x^2) +
...
# ╔═╡ 6189c4b4-c513-42f1-a564-3195dcec4564
md"""
...
# ╔═╡ a4b7df6c-c912-43e5-ab9b-76159e3add9d
begin
    R = 40
    grid = DiscretizedGrid{2}(R, (-5, -5), (5, 5))
end

# ╔═╡ a7247226-a54c-4769-a101-e31283b7e4a8
md"""
...
# ╔═╡ de55a1e0-66f3-4b8b-a227-17562e65e2f0
begin
    function myplotheatmap!(
            plt, f, xlim::Tuple, ylim::Tuple; xlim_box = nothing, ylim_box = nothing)
        x = LinRange(xlim..., 400)
        y = LinRange(ylim..., 400)
        s = heatmap!(plt, x, y, f)
    ...
    function myplotheatmap(
        ...
        plt = plot(xlim = xlim, ylim = ylim, aspect_ratio = :equal,
            xlabel = L"$x$", ylabel = L"$y$")
        myplotheatmap!(plt, func, xlim, ylim; xlim_box = xlim_box, ylim_box = ylim_box)
    end
end

# ╔═╡ da85bc0e-6162-4eac-9169-08a9f2855c3c
myplotheatmap(f, (-5, 5), (-5, 5), xlim_box = (0.25, 1.75), ylim_box = (1.25, 2.75))

# ╔═╡ 968836b2-a1a2-4130-b7f6-d459aaa27c05
myplotheatmap(f, (0.25, 1.75), (1.25, 2.75), xlim_box = (0.94, 1.0), ylim_box = (1.84, 1.9))

# ╔═╡ 84fab71c-4d08-46e9-8f45-0654c28664b9
myplotheatmap(f, (0.94, 1.0), (1.84, 1.9), xlim_box = (0.97, 0.97 + 1e-7),
    ylim_box = (1.88, 1.88 + 1e-7))

# ╔═╡ 93d8a0c4-86df-4009-93e9-42baa11e0c14
let
...
# ╔═╡ d9a8e202-2f32-497c-a977-51cfbb538e1d
md"""
We can now obtain a QTT for `f` in the same way as in the 1D case:"""

# ╔═╡ 2b34bc61-05a3-413c-9746-e7e67076ee13
begin
    # Construct 2D quantics
    qtci, ranks, errors = quanticscrossinterpolate(Float64, f, grid)
    χ = 1:length(qtci.tci.pivoterrors)
    plot(χ, nextfloat(0.0) .+ qtci.tci.pivoterrors ./ qtci.tci.maxsamplevalue,
        xlabel = L"\chi", ylabel = "Normalized error", yscale = :log10,
        ylims = (1e-8, 1e1), yticks = (10.0 .^ (-8:1:1)), legend = false)
end

# ╔═╡ 8fbee7e2-15f7-4f2f-8229-f678a71c7eed
md"""
...
let
    ε = 1e-10
    myplotheatmap(errflog10, (-5, 5 - ε), (-5, 5 - ε),
        xlim_box = (0.25, 1.75), ylim_box = (1.25, 2.75))
end

# ╔═╡ 04d603a0-e42f-439d-81cf-e7d3ce68ab16
myplotheatmap(
    errflog10, (0.25, 1.75), (1.25, 2.75), xlim_box = (0.94, 1.0), ylim_box = (1.84, 1.9))

# ╔═╡ f3fa3855-58cf-4b84-aa89-807dec97524b
myplotheatmap(errflog10, (0.94, 1.0), (1.84, 1.9),
    xlim_box = (0.97, 0.97 + 1e-7), ylim_box = (1.88, 1.88 + 1e-7))

# ╔═╡ 7fb33773-e2c9-45e1-8615-2183a38eb077
let
    xs = LinRange(0.97, 0.97 + 1e-7, 400)
    ys = LinRange(1.88, 1.88 + 1e-7, 400)
...
# ╔═╡ 2117f2a1-c228-41a9-b645-883feb4da9f1
...
manifest_format = "2.0"
project_hash = "f4485ae3a8e9600bb093fc4ea971633928988260"
